# Reference: https://github.com/Ultimaker/Cura/blob/main/plugins/PostProcessingPlugin/scripts/TimeLapse.py

from ..Script import Script


class Position_After_Layer(Script):
    def __init__(self):
        super().__init__()

    def getSettingDataString(self):
        return """{
            "name": "Position After Layer",
            "key": "Position_After_Layer",
            "metadata": {},
            "version": 2,
            "settings":
            {
                ...
                "head_park_z":
                {
                    "label": "Park Print Head z",
                    "description": "What Z location does the head move to for photo.",
                    "unit": "mm",
                    "type": "float",
                    "default_value": 190,
                    "enabled": "park_print_head"
    ...
    def execute(self, data):
        ...
        z_park = self.getSettingValueByKey("head_park_z")
        trigger_command = self.getSettingValueByKey("trigger_command")
        pause_length = self.getSettingValueByKey("pause_length")
        retract = int(self.getSettingValueByKey("retract"))
        gcode_to_append = "; Position After Layer\n"
        last_x = 0
        last_y = 0
        last_z = 0
        ...
        if park_print_head:
            gcode_to_append += self.putValue(G=1, F=feed_rate,
                                             X=x_park, Y=y_park, Z=z_park) + " ; Park print head\n"
